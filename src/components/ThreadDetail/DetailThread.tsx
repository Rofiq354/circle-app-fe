...
import formatTwitterDate from './time';
...
const DetailThread: React.FC<DetailThreadProps> = ({ dataId }) => {
		...
		const fetchData = async () => {
			try {
				...
				const newData = {
					...
					user: result.user,
					isLiked: result.id ? true : false,
				}

				setGetThread(newData);
				// console.log(newData)
  ...
  const data = {
    id: 1,
    user: {
      fullname: "Indah Pra Karya",
      username: "indahpra",
      profile_picture: "https://i.pravatar.cc/100?img=32",
    },
    content: "Kalian pernah ga sih bet on saving? Jadi by calculation sebenernya kita ga survive sampe tanggal tertentu. Tapi entah gimana bisa aja gitu. Ada aja jalannya",
    images: null,
    created_at: "2023-07-26T23:32:00Z",
    likes: 36,
    isLiked: true,
    reply: 291
  };
      ...
      <div className="border-b border-[#333] py-4 px-6">
				...
				<div className="flex items-center gap-3 mb-4">
					<div className="w-12 h-12 rounded-full overflow-hidden shrink-0">
						<img
							src="https://i.pravatar.cc/100?img=32" // Sesuaikan dengan avatar di gambar
							alt="Indah Pra Karya"
					...
					<div className="flex flex-col">
						...
						<span className="text-gray-500 text-[15px] lowercase">@{getThread?.user?.username}</span>
					</div>
				</div>
				...
				<div className="flex items-center gap-6 mt-3">
					...
					<div className="flex items-center gap-2 text-[#777] cursor-pointer hover:text-blue-400 transition-colors">
						...
						<span>{getThread?.replies === 0 ? "" : getThread?.replies} Replies</span>
					</div>
				</div>
      </div>
    </div>
